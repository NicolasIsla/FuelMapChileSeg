...
class UTAE(Decoder):
    def __init__(
        self,
        encoder: Encoder,
        out_channels: int,      # <- antes num_classes
        finetune: bool,
        dec_topology: list,
        output_activation: str = "none",  # "none" | "relu" | "softplus" | "sigmoid"
    ):
        super().__init__(encoder=encoder, num_classes=out_channels, finetune=finetune)
        assert self.finetune

        self.model_name = "UTAE_Regression"
        ...
        self.out_conv = ConvBlock(
            nkernels=[self.dec_topology[0], self.dec_topology[0], out_channels],
            padding_mode="reflect",
            last_relu=False,  # <- importante: no ReLU final por defecto
        )

    def forward(self, x, batch_positions=None, return_att=False):
        input = x["optical"].permute(0, 2, 1, 3, 4)  # B,T,C,H,W
